...
export function ProgramTab() {
  const { programSections, setProgramSections, saveToServer, lastSynced, isSyncing, isReadOnly } = useDashboard();

  const updateSection = (index: number, updated: TrackerSection) => {
    const next = programSections.map((s, i) => (i === index ? updated : s));
    setProgramSections(next);
  };
